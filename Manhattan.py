...
def generate_neighbors(node):
    ...
    for x, y in [(i - 1, j), (i + 1, j), (i, j - 1), (i, j + 1)]:
        if 0 <= x < 3 and 0 <= y < 3:
            ...
            neighbors.append(PuzzleNode(new_state, parent=node, action=(i, j, x, y), cost=node.cost + 1, heuristic=calculate_heuristic(new_state)))
...
def is_solvable(state):
    ...
    inversions = sum(1 for i in range(len(flatten_state) - 1) for j in range(i + 1, len(flatten_state)) if flatten_state[i] > flatten_state[j])
